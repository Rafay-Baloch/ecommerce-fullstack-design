...
const CategorySection = ({ title, bgImage, items }) => {
  return (
    <div className="bg-white border rounded-lg flex flex-col md:flex-row shadow-sm mb-6 overflow-hidden">
      ...
      {/* 1. Left Side: Main Category Banner (with background image) */}
      <div 
      ...
      >
        {/* Overlay agar image zyada dark ya light ho to text parhne ke liye */}
        <div className="relative z-10">
          <h3 className="font-bold text-lg w-24 leading-tight text-black">{title}</h3>
          <button className="bg-white px-4 py-2 rounded-md mt-4 text-xs font-bold shadow-sm hover:bg-gray-100 transition">
            Source now
          </button>
        </div>
      </div>
        ...
        {items.map((item, index) => (
          <div key={index} className="p-4 flex flex-col justify-between hover:bg-gray-50 cursor-pointer h-32 transition">
            ...
            <div className="flex justify-between items-end">
              ...
              {/* 📸 Choti Product Image yahan show hogi */}
              <div className="w-16 h-16 flex items-center justify-center overflow-hidden">
                <img 
                  src={`/images/items/${item.imgName}.png`} 
                  alt={item.name} 
                  className="max-w-full max-h-full object-contain"
                  // Error handling agar image na mile
